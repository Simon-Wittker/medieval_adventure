...
export class Controller {
    ...
    splash_screen_handler() {
        ...
        function startGameFromSplashScreen(){
            helpDiv.style.display = 'block'; 
            sceneDiv.style.display = 'block';
            
            document.body.removeChild(document.getElementById('splash_screen_div'));
        }
    }    
    ...
    update_player_animation(){
        ...
        if (!this.game_end){
            if (!this.model.moving_forward && !this.model.moving_backward && !this.model.attacking && !this.model.blocking && !this.model.open_chest && !this.model.rotating_left && !this.model.rotating_right && !this.model.getting_impact){
                ...
                impact.stop()
                chest_opening.stop();
                if (this.model.player_stamina >= 1.25){
                    this.model.player_stamina -= 1.25;
                } else {
                    this.model.move_speed /= 2;
